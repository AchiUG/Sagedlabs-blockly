...
const BlocklyWorkspace = forwardRef<BlocklyWorkspaceHandle, BlocklyWorkspaceProps>(({
  ...
  useEffect(() => {
     ...
     * Defensive checks added to prevent crashes if variable names are missing.
     */
    const variableCategoryCallback = (workspace: any) => {
      ...
      if (variableList && variableList.length > 0) {
        ...
        // Defensive check: Filter out variables with missing names
        const validVariables = variableList.filter((v: any) => v && v.name);
        
        const sortedVariables = [...validVariables].sort((a, b) => 
          String(a.name).localeCompare(String(b.name), undefined, { sensitivity: 'base' })
        );

        const seenNames = new Set<string>();
        for (const variable of sortedVariables) {
          const nameLower = String(variable.name).toLowerCase().trim();
